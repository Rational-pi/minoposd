...
void writePanels()
{
    ...
    if (setup_menu_active) {
        ...
        if (panel < npanels) { // first or second panel
            ...
            if (osd_got_home) {
                if (ISc(panel, Hor_BIT)) {
                    panUAVPosition(panHorizon_XY[0][panel] + 6, panHorizon_XY[1][panel] + 2);
                }
            }
#endif
        } else { // off panel
...
void panFlightMode(int first_col, int first_line)
{
    ...
    if (osd_mode == 0) {
        ...
        mode_str = "at "; // AUTOTUNE
    } else if (osd_mode == 5) {
        mode_str = "alh"; // ALTITUDEHOLD
    } else if (osd_mode == 6) {
        mode_str = "alv"; // ALTITUDEVARIO
    } else if (osd_mode == 7) {
        mode_str = "vc "; // VELOCITYCONTROL
    } else if (osd_mode == 8) {
    ...
    if (osd_mode == 0) {
        ...
        mode_str = "pvf"; // POSITIONVARIOFPV
    } else if (osd_mode == 10) {
        mode_str = "pvl"; // POSITIONVARIOLOS
    } else if (osd_mode == 11) {
        mode_str = "pvd"; // POSITIONVARIONSEW
    } else if (osd_mode == 12) {
        mode_str = "rtb"; // RETURNTOBASE
    } else if (osd_mode == 13) {
        mode_str = "lan"; // LAND
...
void panOtherUAV(int first_col, int first_line)
{
    ...
    if (oUAV_bearing < 0) {
        oUAV_bearing += 360; // normalization
    }
