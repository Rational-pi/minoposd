...
#define P_OFFSET 100 // [us]	PWM offset for detecting stick movement
    ...
    if (chan2_raw > chan2_r_middle + P_OFFSET) {
        osd_lat -= LAT_STEPS;
    } else if (chan2_raw < chan2_r_middle - P_OFFSET) {
        osd_lat += LAT_STEPS;
    }

    if (chan1_raw > chan1_r_middle + P_OFFSET) {
        osd_lon += LON_STEPS;
    } else if (chan1_raw < chan1_r_middle - P_OFFSET) {
        osd_lon -= LON_STEPS;
    }
